...
// main client loops
pub fn send_and_receive(host: String, port: String, nick: String) -> Result<()> {
    ...
    // reading loop
    let reading_thread: JoinHandle<Result<()>> = thread::spawn(move || loop {
        let stdin = io::stdin();
    ...
    // receiving and sending loop
    let receive_and_send: JoinHandle<Result<()>> = thread::spawn(move || loop {
        thread::sleep(Duration::from_millis(50));

        // check if anything arrived from server
        if let Ok(m) = handle_client(&mut connection) {
            let (nick_incoming, message, timestamp) = m.destructure();

            // replace client message side with
            // ISSUE - incoming message will delete a message currently being written
            // will be fixed in UI homework
            clear_last_line()?;
            ...
            match message {
                ...
                MessageType::Image(_) => {
                    ...
                    receive_and_save(message, nick_receive.as_str())?;
                }
        ...
        while let Ok(my_message) = rx.try_recv() {
            if let Err(e) = send_message(&mut connection, &my_message) {
                log::error!("{}", e);
                // print nick and keep cursor on the same line
                print_nick(nick_receive.as_str(), None)?
            };
        }
    });
...
fn receive_and_save(message: MessageType, nick: &str) -> Result<()> {
...
fn print_nick(nick: &str, timestamp: Option<String>) -> Result<()> {
...
fn move_to_message() -> Result<()> {
    execute!(io::stdout(), cursor::MoveToColumn(30),)?;
    Ok(())
}
...
fn replace_last_line(nick: &str, input: &str) -> Result<()> {
    clear_previous_line();
